"""
Module for audio diarization and transcription using OpenAI GPT-5 with native audio input.

Approach: GPT-5 / GPT-5 mini natively accept audio input, so we send audio directly
to the model (similar to how Gemini handles it) for transcription, diarization,
and field extraction. Supports multiple Indian languages.
"""
import os
import json
import time
...
class DiarizationProcessor:
    def __init__(self, message, metadata_path, audio_path, logger=None):
        """Initialize the DiarizationProcessor with OpenAI GPT-5 configuration."""
        self.logger = logger
        if self.logger:
            self.logger.info("Initializing DiarizationProcessor with OpenAI GPT-5 (native audio)")
        ...
        # GPT-5 supports native audio input — use as primary model
        self.model_name = os.getenv("OPENAI_MODEL", "gpt-5")
        # Mini variant for cost-effective operations
        self.model_name_mini = os.getenv("OPENAI_MODEL_MINI", "gpt-5-mini")
        ...
        self.system_prompt = ""
        self.audio_base64 = ""  # Store base64 audio for reuse in extract_fields
        try:
    ...
    def call_model(self, content, extract_model=None, generation_config=None, model_name=None, system_instruction=None):
        """
        Call the OpenAI GPT-5 model to generate content.
        Uses Chat Completions API with native audio input support.
        
        Args:
            content: The content to process (can include audio data and prompts)
        ...
        if isinstance(content, list):
            # Handle multimodal content (audio + text)
            user_content = []
            for item in content:
                if isinstance(item, dict) and item.get("type") in ("input_audio", "text"):
    ...
    def transcribe_chunk(self, idx, chunk_path, past_reference=None):
        """
        Transcribe a single audio chunk using GPT-5 with native audio input.
        
        GPT-5 accepts audio directly (like Gemini), so we encode the audio as base64
        and send it in the Chat Completions API with input_audio type.
        Supports multiple Indian languages — the model auto-detects and translates to English.
        """
        system_prompt = f"""
        ...
        **IMPORTANT: The audio may be in any Indian language including but not limited to: Hindi, Tamil, Telugu, Kannada, Malayalam, Bengali, Marathi, Gujarati, Punjabi, Urdu, Odia, Assamese, Konkani, Maithili, Dogri, Kashmiri, Nepali, Sindhi, Manipuri, or English. You must accurately transcribe and translate ALL languages to English.**
        ...
        prompt = f"""
        **IMPORTANT : I want the translation in english language Only. The audio may be in any Indian language — translate everything to English.**\n\n\n
        ...
        **IMPORTANT: Make Sure Time Start and End are in the format of MM:SS.MS only. Do not try to be over smart and convert it to HH:MM:SS.MS. or any other format.**
        ...
        if past_reference is not None:
            prompt += past_reference

        # Encode audio as base64 for GPT-5 native audio input
        audio_base64 = self._encode_audio_base64(chunk_path)
        audio_format = self._get_audio_format(chunk_path)
        self.audio_base64 = audio_base64
        self.audio_format = audio_format
        
        # Create content with system instruction embedded in the prompt
        full_prompt = f"{system_prompt}\n\n{prompt}"
        
        # GPT-5 native audio: send audio directly via input_audio type
        messages = [
            {
                "role": "user",
                "content": [
        ...
        if self.logger:
            self.logger.info(f"Successfully transcribed chunk {idx} with {len(json_data)} segments")
    ...
    def transcribe_with_openai(self, audio_path, duration):
        """Transcribe audio using OpenAI GPT-5 model with native audio input."""
    ...
    def extract_fields(self, instruction, system_prompt, extract_model, mode = "transcript", audio_path=None):
        """
        Extract fields using OpenAI GPT-5.
        
        For audio mode: sends audio directly to GPT-5 (native audio support).
        For transcript mode: sends text only via Chat Completions API.
        """
        ...
        if mode == "audio" and self.audio_base64:
            # GPT-5 native audio: send audio directly
            messages = [
                {
                    "role": "user",
                    "content": [
